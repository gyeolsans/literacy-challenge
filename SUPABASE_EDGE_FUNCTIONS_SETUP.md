...
현재 배포 버전 표식은 `full-supabase-repair-v1`입니다.
...
```text
DEPLOY_VERSION full-supabase-repair-v1
APP_CONFIG_AT_START ...
```

[index.html](./index.html)은 로컬 스크립트에 `?v=full-supabase-repair-v1` 쿼리를 붙여 Vercel/브라우저 캐시가 오래된 `script.js`를 잡는 일을 줄입니다.
